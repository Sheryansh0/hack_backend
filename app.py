...
def convert_float32_to_float(data):
    """Recursively convert float32 values to native Python float."""
    if isinstance(data, dict):
        ...
        return float(data)  # Convert float32 to float
    else:
        return data
    ...
    if request.method == "POST":
        if "job_description" in request.form:
            ...
            if job_description:
                ...
                return jsonify(job_details)  # Return job details as JSON
            ...
            # Convert float32 values to native Python float before returning JSON
            screened_resumes_converted = convert_float32_to_float(screened_resumes)
            return jsonify({"screened_resumes": screened_resumes_converted})  # Return screened resumes as JSON
...
def clear_resumes():
    global screened_resumes
    screened_resumes = []
    return jsonify({"message": "Cleared all resumes."})
